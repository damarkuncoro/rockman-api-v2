...
export async function GET(
  ...
  try {
    ...
    // Ambil semua customers dan filter berdasarkan user ID
    const allCustomers = await customersService.GET.All();
    ...
    if (status) {
      filteredCustomers = filteredCustomers.filter(customer => customer.customerStatus === status);
    }

    if (priority) {
      filteredCustomers = filteredCustomers.filter(customer => customer.customerTier === priority);
    }

    if (category) {
      filteredCustomers = filteredCustomers.filter(customer => customer.customerType === category);
    }
