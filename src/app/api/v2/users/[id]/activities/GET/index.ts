/**
 * GET handler untuk /api/v2/users/[id]/activities
 * 
 * Endpoint untuk mendapatkan daftar aktivitas berdasarkan user ID
 */

import { NextRequest, NextResponse } from 'next/server';
import { StatusCodes } from 'http-status-codes';
import { accessLogsService } from '@/v2/services/database/access_logs';
import { usersService } from '@/v2/services/database/users';

/**
 * GET handler untuk mengambil aktivitas berdasarkan user ID
 * 
 * @param request - NextRequest object
 * @param params - Parameter dari URL (id)
 * @returns NextResponse dengan data aktivitas atau error
 */

export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const resolvedParams = await params;
    const userId = resolvedParams.id;

    // Validasi user ID
    const user = await usersService.GET.ById(userId);
    if (!user) {
      return NextResponse.json(
        { error: "User tidak ditemukan" },
        { status: StatusCodes.NOT_FOUND }
      );
    }

    // Ambil parameter query untuk pagination
    const searchParams = request.nextUrl.searchParams;
    const page = parseInt(searchParams.get('page') || '1');
    const limit = parseInt(searchParams.get('limit') || '10');

    // Ambil semua access logs dan filter berdasarkan user ID
    const allLogs = await accessLogsService.GET.All();
    const userLogs = allLogs.filter(log => log.userId === userId);

    // Implementasi pagination di level aplikasi
    const startIndex = (page - 1) * limit;
    const endIndex = page * limit;
    
    // Hitung total untuk pagination
    const totalCount = userLogs.length;

    // Buat response
    return NextResponse.json({
      success: true,
      data: userLogs.slice(startIndex, endIndex),
      meta: {
        pagination: {
          page,
          limit,
          total: totalCount,
          totalPages: Math.ceil(totalCount / limit)
        }
      }
    }, { status: StatusCodes.OK });
  } catch (error) {
    console.error("Error fetching user activities:", error);
    return NextResponse.json(
      { error: "Terjadi kesalahan saat mengambil data aktivitas" },
      { status: StatusCodes.INTERNAL_SERVER_ERROR }
    );
  }
}